...
export class Dropdown implements MenuElement {
  ...
  render(view: EditorView) {
    ...
    label.addEventListener("mousedown", e => {
      e.preventDefault()
      const winW = win.globalThis.innerWidth
      ...
      if (open) {
        ...
        let dropX = wrap.getBoundingClientRect().x + 4
        const dropW = open.node.getBoundingClientRect().width
        if ((dropX + dropW) > winW) {
          dropX = winW - open.node.getBoundingClientRect().width
        }

        open.node.style.left = dropX + "px"
